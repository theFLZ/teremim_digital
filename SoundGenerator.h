...
public:
  ...
  //FELIPE::  distorções
  void setDistorcao(int distorc_n){
    this->distorcao = distorc_n;
  }

  // ruído gaussiano
  float randomGaussian(float mean, float stddev, float minVal, float maxVal) {
  ...
  // FELIPE: função criada para obter a função de onda com base na distorção setada
  float getFuncao(float m_aplitude, float angle){
    
    float retorno;
    
    if (this->distorcao == 1) {
      // hardclipping
      retorno = sinf(angle); //primeiro obtenho apenas o seno do angulo atual
      if (retorno > 0.7f) { // faço a comparação em relação ao ponto de corte
        retorno = 0.7f;
      } else if (retorno < -0.7f)
      {
        retorno = -0.7;
      }

      retorno *= m_amplitude; //somente depois faço a multiplicação pela amplitude
    }
    
    else if (this->distorcao == 2){
      retorno = m_amplitude * sinf(angle) * sinf(5*angle);
    } 
    
    else if (this->distorcao == 3){
      retorno = m_amplitude * (sinf(angle) + sinf(5*angle))/2;
    } 
    
    else if (this->distorcao == 4){
      // ruído branco realmente... não serve pra muita coisa
      //retorno = m_amplitude * sinf(angle + (randomGaussian(0, 20, -200, 200)/100));

      retorno = m_amplitude * sinf(angle + tanf(angle));
    }
    else if (this->distorcao == 5){
      
      // hardclipping + multiplicação por outra senoide mais rápida
      retorno = sinf(angle);
      if (retorno > 0.2f){
        retorno *= sinf(3*angle);
      } else {
        retorno *= sinf(4*angle)/2;
      }

      retorno *= m_amplitude;
    }
    else if (this->distorcao == 6){
      
      // hardclipping + multiplicação por outra senoide mais lenta e rápida
      retorno = sinf(angle);
      if (retorno > 0.2f){
        retorno = sinf(3*angle);
      } else {
        retorno = sinf(angle/4);
      }

      retorno *= m_amplitude;
    }

    else if (this->distorcao == 7){
      
      // hardclipping + multiplicação por outra senoide mais lenta
      retorno = sinf(angle);
      if (retorno > 0.2f){
        retorno *= sinf(angle/3);
      } else {
        retorno = sinf(angle/4);
      }
    ...
    else if (this->distorcao == 8){
      retorno = m_amplitude * sinf(2*angle) * sinf((20/3)*angle) * sinf(42*angle);
    }

    else if (this->distorcao == 9){
      retorno = m_amplitude * (sinf(2*angle) + sinf((20/3)*angle) + sinf(42*angle))/3;
    }

    else if (this->distorcao == 10) {
      retorno = sinf(angle);
      if ((retorno > 0.4) || (retorno < -0.4)) {
        retorno *= m_amplitude * sinf((20/3)*angle) * sinf(42*angle);
      }
    }

    else {
      retorno = m_amplitude * sinf(angle);
    }
  ...
  virtual T readSample() override {
    ...
    T result = this->getFuncao(m_amplitude,angle); // m_amplitude * sinf(angle); //FELIPE:: alterado para ter a distorção
